...
const Navbar = () => {
  ...
  const [isNotificationCentered, setIsNotificationCentered] = useState(false);
  ...
  // ฟังก์ชันสำหรับการคลิกที่ปุ่ม notification เพื่อเปิดแบบกลางจอ
  const handleNotificationClick = (e) => {
    e.preventDefault();
    e.stopPropagation();
    setIsNotificationCentered(true);
  };
  ...
  if (isAdmin) {
    return (
      <>
            ...
            <div className="flex justify-between items-center h-16">
              ...
              <div className="flex items-center space-x-4">
                ...
                <div className="flex items-center space-x-2">
                  {/* ปุ่ม notification แบบปกติ - จะถูกเปลี่ยนเป็นการแสดงตรงกลางเมื่อคลิก */}
        ...
        {/* Notification Center Overlay - แสดงเมื่อคลิกที่ notification */}
        {isNotificationCentered && (
          <div 
            className="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 flex items-center justify-center"
            onClick={() => setIsNotificationCentered(false)}
          >
            <div 
              className="relative"
            ...
            >
              <NotificationDropdown 
                ref={notificationRef} 
                isCentered={true}
                onClose={() => setIsNotificationCentered(false)}
              />
            </div>
          </div>
        )}
      ...
      <nav className="w-full bg-white shadow-lg">
        <div className="max-w-[1920px] mx-auto px-4 lg:px-[140px]">
          <div className="flex justify-between items-center h-24">
            ...
            {/* Mobile Top Right Menu and Actions */}
            <div className="flex items-center space-x-2 md:hidden">
              {/* ปุ่ม notification บนมือถือ - จะถูกเปลี่ยนเป็นการแสดงตรงกลางเมื่อคลิก */}
              <div onClick={handleNotificationClick}>
                <NotificationDropdown ref={notificationRef} />
              </div>
              
              {/* Saved Activities Folder on Mobile Top */}
              <div className="relative">
                <button 
                  onClick={() => setIsDropdownVisible(!isDropdownVisible)}
                  className="p-2 rounded-full hover:bg-gray-100 shadow-sm relative"
                >
                  <Folder size={22} className="text-gray-600 hover:text-[#3BB77E]" />
                  {savedActivities.length > 0 && (
                ...
                {isDropdownVisible && <SavedActivitiesDropdown />}
              </div>
            ...
            <div className="px-2 pt-2 pb-3 space-y-1 bg-white shadow-lg rounded-lg">
              ...
              {savedActivities.length > 0 && (
                <div className="px-3 py-2">
                  <button
                    ...
                    className="w-full flex items-center text-gray-600 hover:text-[#3BB77E]"
                  >
              ...
              {user ? (
                ...
                <div className="px-3 py-2">
                  <Link
                    to="/login"
                    className="flex items-center justify-center w-full px-3 py-2 bg-[#3BB77E] text-white rounded-md hover:bg-[#33a96e] shadow-sm"
                    onClick={() => setIsMobileMenuOpen(false)}
                  >
